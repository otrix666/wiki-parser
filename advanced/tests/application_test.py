...
from advanced.app.application import WikiParser
from advanced.app.db import Database
from advanced.app.errors import DbError, HttpError
...
@pytest.mark.asyncio
async def test_success_add_urls(wiki_parser: WikiParser) -> None:
...
@pytest.mark.asyncio
async def test_failure_add_urls(wiki_parser: WikiParser) -> None:
...
@pytest.mark.asyncio
async def test_success_get_html_content(wiki_parser: WikiParser) -> None:
...
@pytest.mark.asyncio
async def test_field_get_html_content(wiki_parser: WikiParser) -> None:
    test_url = {"http://wiki.test.com"}
    content = """<html>
            <body>
                <a href="/wiki/Programming">Programming</a>
    ...
    wiki_parser._http_client.get_content.side_effect = HttpError("http error")
