...
def parse_wiki_page(
    ...
    for url in urls:
        try:
            ...
            next_urls = {correct_url for correct_url in page_urls if correct_url and re.match(r'(https://(?:[a-z]{2}\.)?wikipedia\.org/wiki/[^\s%">\\]+)', correct_url)}
    ...
    parse_wiki_page(logger=logger, db=db, redis_cli=redis_cli, http_cli=http_cli, urls=next_urls, max_depth=max_depth, current_depth=current_depth + 1)
